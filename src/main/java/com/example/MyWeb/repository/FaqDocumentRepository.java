...
public interface FaqDocumentRepository extends JpaRepository<FaqDocument, Long> {
         ...
         * Toán tử <=> tính khoảng cách cosine (gần 0 = giống nhau, gần 2 = hoàn toàn
         * khác nhau).
        ...
        @Query(value = "INSERT INTO faq_documents (category, content, created_at, embedding, title) " +
                        "VALUES (:category, :content, NOW(), CAST(:embedding AS vector), :title)", nativeQuery = true)
         ...
         */
        @Modifying
        @Query(value = "UPDATE faq_documents SET embedding = CAST(:embedding AS vector) WHERE id = :id", nativeQuery = true)
